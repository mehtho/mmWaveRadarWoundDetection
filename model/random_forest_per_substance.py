...
# Model evaluation
# -----------------------------
def run_rf_cv(X, y, title="Experiment"):
    print("\n=====================================")
    print(f"   {title}")
    print("=====================================")
...
def run_water_vs_jelly(X, y):
    ...
    run_rf_cv(X_sub, y_bin, title="WATER VS JELLY (2.5/5.0 vs 3.75/7.5)")
...
def run_pairwise_experiment(
    ...
    run_rf_cv(X_sub, y_pair, title=title)
...
def run_dry_water5_jelly75(X, y):
    ...
    run_rf_cv(X_sub, y_tri, title="DRY (0.0) vs WATER 5.0 vs JELLY 7.5")
...
def main():
    ...
    run_rf_cv(Xw, yw, title="WATER ONLY (0, 2.5, 5.0)")

    # 2) JELLY ONLY (0, 3.75, 7.5)
    Xj, yj = filter_by_labels(X, y, JELLY_LABELS)
    run_rf_cv(Xj, yj, title="JELLY ONLY (0, 3.75, 7.5)")

    # 3) ALL LABELS (original)
    run_rf_cv(X, y, title="ALL LABELS (0, 2.5, 3.75, 5.0, 7.5)")
